...
async function openTeacherResultReview(resultId) {
  ...
  try {
    ...
    el('teacher-result-review-content').innerHTML = `<section class="result-review-view"><p class="eyebrow">Проверка открытых ответов</p><h2>${escapeHtml(worksheet?.title || homework.title || 'Worksheet')}</h2><div class="result-review-meta"><span><b>Ученик:</b> ${escapeHtml(student.name)}</span><span><b>Автопроверка:</b> ${Number(result.score || 0)}/${Number(result.max_score || 0)} · ${Number(result.percentage || 0)}%</span></div>${answers.length ? answers.map((answer, index) => { const exercise = exerciseById.get(answer.exercise_id); const content = parseContent(exercise?.content); const instruction = exercise?.instruction || answer.prompt || ''; const prompt = content?.prompt && content.prompt !== instruction ? content.prompt : ''; return `<article class="result-review-answer"><h3>Открытый ответ ${index + 1}</h3>${instruction ? `<p><b>Инструкция:</b> ${escapeHtml(instruction)}</p>` : ''}${prompt ? `<p><b>Задание:</b> ${escapeHtml(prompt)}</p>` : ''}<div class="student-open-answer"><b>Ответ ученика</b><p>${escapeHtml(answer.answer || 'Ответ не указан')}</p></div></article>`; }).join('') : '<p class="empty-state">Открытые ответы не найдены.</p>'}</section>`;
