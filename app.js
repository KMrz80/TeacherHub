...
function showBuilderPreview() { const nodes = [...document.querySelectorAll('.exercise-editor')], editorExercises = nodes.map((node, order) => ({ id: `preview-${order}`, ...readWorksheetExercise(node, order) })).filter(Boolean), exercises = state.previewExercises?.length ? state.previewExercises : editorExercises; const form = el('worksheet-form'); el('worksheet-preview-content').innerHTML = worksheetPageMarkup({ title: form.elements.title.value || 'Worksheet', instructions: form.elements.work_goal.value, estimated_time: state.editingWorksheet?.estimated_time || form.elements.estimated_time.value }, exercises, true); bindWorksheetInteractions(el('worksheet-preview-content')); el('worksheet-preview-dialog').showModal(); }
