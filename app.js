...
async function openTeacherResultReview(resultId) {
  ...
  try {
    ...
    const completeButton = el('complete-result-review'); completeButton.dataset.resultId = result.id; completeButton.disabled = result.status !== 'needs_review'; completeButton.textContent = result.status === 'needs_review' ? 'Принять / Завершить проверку' : 'Проверка завершена'; completeButton.onclick = () => completeTeacherResultReview(result.id);
    el('teacher-result-review').showModal();
  } catch (error) { handleFatal(error); } finally { setLoading(false); }
}

async function completeTeacherResultReview(resultId) {
  setLoading(true);
  try {
    const updated = await pb.collection('homework_results').update(resultId, { status: 'reviewed' });
    state.teacherData.results = state.teacherData.results.map((item) => item.id === resultId ? updated : item);
    el('teacher-result-review').close(); renderTeacher(); toast('Проверка завершена');
  } catch (error) { handleFatal(error); } finally { setLoading(false); }
}
