...
function renderTeacher() {
  ...
  bindTeacherProgressControls(); el('add-student').addEventListener('click', () => el('student-dialog').showModal()); el('cancel-student').addEventListener('click', () => el('student-dialog').close()); el('student-form').addEventListener('submit', saveStudent); animateBars();
}
...
async function openWorksheetDraft(id) {
  ...
  try {
    ...
    const form = el('worksheet-form'); form.elements.student.value = draft.student || state.editingHomework?.student || ''; form.elements.title.value = draft.title || ''; form.elements.work_goal.value = draft.instructions || draft.focus || ''; form.elements.estimated_time.value = draft.estimated_time || ''; form.elements.due_date.value = datetimeLocalValue(draft.due_date || state.editingHomework?.due_date);
...
function showBuilderPreview() { const nodes = [...document.querySelectorAll('.exercise-editor')], editorExercises = nodes.map((node, order) => ({ id: `preview-${order}`, ...readWorksheetExercise(node, order) })).filter(Boolean), exercises = state.previewExercises?.length ? state.previewExercises : editorExercises; const form = el('worksheet-form'); el('worksheet-preview-content').innerHTML = worksheetPageMarkup({ id: state.editingWorksheet?.id || '', title: form.elements.title.value || 'Worksheet', instructions: form.elements.work_goal.value, estimated_time: state.editingWorksheet?.estimated_time || form.elements.estimated_time.value }, exercises, true); bindWorksheetInteractions(el('worksheet-preview-content')); el('worksheet-preview-dialog').showModal(); }
...
async function ensureAgentWorksheetDraft() {
  const form = el('worksheet-form'), workGoal = form.elements.work_goal.value.trim(); if (!workGoal) { form.elements.work_goal.setCustomValidity('Заполните поле «Что отработать».'); form.elements.work_goal.reportValidity(); form.elements.work_goal.setCustomValidity(''); throw new Error('Заполните поле «Что отработать».'); } if (!form.reportValidity()) throw new Error('Заполните обязательные поля worksheet.');
...
function worksheetPageMarkup(worksheet, exercises, preview = false, savedStatus = '') { const assessable = exercises.filter(isAssessableExercise), reviewable = exercises.filter((exercise) => exercise.type === 'open_text_teacher_review'); return `<form ${preview ? '' : 'id="worksheet-player"'} class="worksheet-page student-worksheet"><header class="worksheet-intro">${preview && worksheet.id ? `<p class="worksheet-label">Worksheet ID: ${escapeHtml(worksheet.id)}</p>` : ''}<p class="worksheet-label">Interactive worksheet</p><h2>${escapeHtml(worksheet.title || 'Worksheet')}</h2>${worksheet.instructions ? `<p class="worksheet-goal">${escapeHtml(worksheet.instructions)}</p>` : ''}<div class="worksheet-meta">${worksheet.estimated_time ? `<span>Время: ${escapeHtml(worksheet.estimated_time)}</span>` : ''}<span id="worksheet-progress">Выполнено 0 из ${assessable.length}</span></div></header><main class="worksheet-tasks">${exercises.map(worksheetExerciseMarkup).join('')}</main><footer class="worksheet-submit"><button class="primary-button" type="${preview ? 'button' : 'submit'}" ${assessable.length || reviewable.length ? '' : 'disabled'}>Finish worksheet</button><strong id="worksheet-score">${escapeHtml(savedStatus)}</strong></footer></form>`; }

function exerciseItems(exercise) { const content = exercise.content || {}; if (Array.isArray(content.items) && content.items.length) return content.items; return [content]; }
function multipleChoiceMarkup(exercise) { return exerciseItems(exercise).map((item, index) => { const options = Array.isArray(item.options) ? item.options : (exercise.content?.options || []), question = item.question || item.text || (index ? '' : exercise.content?.question || ''); return `<fieldset class="worksheet-item mc-item" data-item-index="${index}"><legend><span>${index + 1}.</span> ${escapeHtml(question)}</legend><div class="choice-grid">${options.map((option) => `<label class="option"><input type="radio" name="ws-${exercise.id}-${index}" value="${escapeAttr(option)}"><span>${escapeHtml(option)}</span></label>`).join('')}</div></fieldset>`; }).join(''); }
function dropdownGapData(item) { const gaps = []; Object.keys(item).filter((key) => /^gap\d+_options$/.test(key)).sort((a, b) => Number(a.match(/\d+/)[0]) - Number(b.match(/\d+/)[0])).forEach((key) => { const number = key.match(/\d+/)[0]; gaps.push({ key: `gap${number}`, options: item[key] || [], correct_answer: item[`gap${number}_correct`] }); }); if (!gaps.length && Array.isArray(item.options)) gaps.push({ key: 'gap1', options: item.options, correct_answer: item.correct_answer }); return gaps; }
function dropdownItemMarkupStudent(exercise, item, itemIndex) { if (item.text_before !== undefined || item.text_after !== undefined) return `<div class="dropdown-sentence" data-item-index="${itemIndex}"><span>${escapeHtml(item.text_before || '')}</span>${dropdownSelectMarkup(exercise, itemIndex, 0, item.options || [])}<span>${escapeHtml(item.text_after || '')}</span></div>`; const gaps = dropdownGapData(item); let text = escapeHtml(item.text || item.question || ''); gaps.forEach((gap, gapIndex) => { const select = dropdownSelectMarkup(exercise, itemIndex, gapIndex, gap.options); const patterns = [new RegExp(`\\[${gap.key}\\]`, 'i'), new RegExp(`\\{${gap.key}\\}`, 'i'), /_{2,}/]; const pattern = patterns.find((candidate) => candidate.test(text)); text = pattern ? text.replace(pattern, select) : `${text} ${select}`; }); return `<div class="dropdown-sentence" data-item-index="${itemIndex}">${text}</div>`; }
...
function reorderItems(exercise) { return exerciseItems(exercise).map((item) => { const answer = item.correct_answer ?? exercise.correct_answer; const correct = Array.isArray(answer) ? answer : String(answer || '').match(/[\p{L}\p{N}'’]+|[^\s\p{L}\p{N}'’]/gu) || []; return { prompt: item.question || item.text || '', words: Array.isArray(item.words) ? item.words : (exercise.content?.words || []), correct }; }); }
...
function worksheetExerciseMarkup(exercise, index) {
  ...
  if (exercise.type === 'open_text_teacher_review') { const saved = savedOpenTextAnswer(exercise.id), support = Array.isArray(content.support) ? content.support : String(content.support || '').split('\n').filter(Boolean), criteriaRaw = content.success_criteria || content.success_criterion || [], criteria = Array.isArray(criteriaRaw) ? criteriaRaw : [criteriaRaw]; answer = `<div class="open-text-review">${content.prompt ? `<h4>${escapeHtml(content.prompt)}</h4>` : ''}${support.length ? `<aside class="answer-support"><strong>You can use:</strong><ul>${support.map((line) => `<li>${escapeHtml(line)}</li>`).join('')}</ul></aside>` : ''}<textarea name="open-${exercise.id}" rows="7" placeholder="${escapeAttr(content.placeholder || 'Type your answer here…')}">${escapeHtml(saved)}</textarea>${criteria.filter(Boolean).length ? `<div class="success-box"><strong>Success criterion</strong><ul>${criteria.filter(Boolean).map((criterion) => `<li>${escapeHtml(criterion)}</li>`).join('')}</ul></div>` : ''}<p class="teacher-review-note">Teacher will review.</p></div>`; }
  return `<section class="worksheet-exercise task-section" data-exercise="${exercise.id}"><div class="task-heading"><span class="exercise-number">${index + 1}</span><div><span class="task-label">Task ${index + 1}</span><h3>${escapeHtml(exercise.instruction)}</h3></div></div><div class="worksheet-answer">${answer}</div><p class="feedback" aria-live="polite"></p></section>`;
}
