...
function reorderItems(exercise) { return exerciseItems(exercise).map((item) => { const answer = item.correct_answer ?? item.content?.correct_answer ?? exercise.correct_answer, rawWords = item.words ?? item.content?.words ?? exercise.content?.words ?? [], words = Array.isArray(rawWords) ? rawWords : String(rawWords).split(/\s+/).filter(Boolean), correct = Array.isArray(answer) ? answer : String(answer || '').match(/[\p{L}\p{N}'’]+|[^\s\p{L}\p{N}'’]/gu) || []; return { prompt: item.question || item.text || item.prompt || '', words, correct }; }); }
