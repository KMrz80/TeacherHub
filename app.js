...
function renderParentHome() {
  ...
  <section class="card progress-card"><div class="card-title"><h2>Последний результат ДЗ</h2></div>${result ? `<div class="result-strip"><div><strong>${escapeHtml(result.expand?.homework?.title || 'Домашнее задание')}</strong><p class="muted">Выполнено ${formatDate(result.completed_at, true)}</p>${result.status === 'returned' && result.teacher_comment ? `<p class="teacher-return-comment">Комментарий преподавателя: ${escapeHtml(result.teacher_comment)}</p>` : ''}</div><div class="result-badge">${result.status === 'needs_review' ? 'На проверке' : result.status === 'returned' ? 'Нужно исправить' : `${result.percentage}%`}</div></div>` : '<div class="empty-state">Пока нет результатов</div>'}</section>`;
...
function homeworkCard() {
  ...
  return `<section class="card homework-card"><div class="card-title"><div class="icon-box">▤</div><h2>Домашние задания</h2></div><div class="homework-list">${homeworks.map((homework) => { const result = homeworkResultFor(homework.id); return `<article class="homework-list-item"><div><h3 class="homework-title">${escapeHtml(homework.title)}</h3>${homework.instructions ? `<p class="homework-instructions">${escapeHtml(homework.instructions)}</p>` : ''}<p class="muted">Срок: ${formatDate(homework.due_date, true)} · Опубликовано</p>${result ? `<strong class="homework-result-status">${result.status === 'needs_review' ? `Выполнено: ${result.score}/${result.max_score} · ожидает проверки` : result.status === 'returned' ? 'Нужно исправить' : `Выполнено: ${result.score}/${result.max_score}`}</strong>${result.status === 'returned' && result.teacher_comment ? `<p class="teacher-return-comment">Комментарий преподавателя: ${escapeHtml(result.teacher_comment)}</p>` : ''}` : '<span class="muted">Не выполнено</span>'}</div><button class="primary-button open-homework" data-homework-id="${homework.id}" type="button">${result?.status === 'returned' ? 'Исправить' : 'Открыть'} &nbsp;→</button></article>`; }).join('')}</div></section>`;
}
...
function renderTeacher() {
  ...
  el('content').innerHTML = `<section class="card table-card"><div class="card-title"><h2>Ученики</h2><button id="add-student" class="primary-button" type="button">+ Добавить ученика</button></div><table class="student-table"><thead><tr><th>Ученик</th><th>Текущая тема</th><th>Прогресс</th><th>Действия</th><th>Текущее ДЗ</th><th>Последний результат</th></tr></thead><tbody>${state.students.map((student) => {
    ...
    return `<tr data-student-row="${student.id}"><td><strong>${escapeHtml(student.name)}</strong><div class="muted">${escapeHtml(student.course || 'English')} · ${escapeHtml(student.level || '—')}</div></td><td>${escapeHtml(student.current_topic || '—')}</td><td>${isEditing ? progressEditor(progress) : teacherProgressMarkup(progress)}</td><td>${isEditing ? `<div class="edit-actions"><button class="primary-button save-progress" data-student-id="${student.id}" type="button">Сохранить</button><button class="secondary-button cancel-progress" type="button">Отмена</button></div>` : `<div class="row-actions"><button class="secondary-button edit-progress" data-student-id="${student.id}" type="button">Изменить прогресс</button></div>`}</td><td>${homeworks.length ? `<div class="teacher-homework-list">${homeworks.map((homework) => { const homeworkResult = d.results.find((item) => item.homework === homework.id && item.student === student.id); const needsReview = homeworkResult?.status === 'needs_review'; return `<article><button class="teacher-homework-link" data-homework-id="${homework.id}" type="button">${escapeHtml(homework.title)}</button><div class="homework-preview-actions"><span class="muted">до ${formatDate(homework.due_date)} · ${escapeHtml(homework.status)}</span><button class="secondary-button open-teacher-homework" data-homework-id="${homework.id}" type="button">Открыть</button><button class="danger-button delete-homework" data-homework-id="${homework.id}" type="button">Удалить</button></div>${needsReview ? '<p class="review-delete-hint">Сначала завершите проверку результата — нажмите «Проверить» в колонке результата.</p>' : homeworkResult ? `<p class="homework-result-summary">${homeworkResult.status === 'reviewed' ? 'Проверено' : 'Выполнено'}: ${Number(homeworkResult.score || 0)}/${Number(homeworkResult.max_score || 0)} · ${Number(homeworkResult.percentage || 0)}%</p>` : ''}</article>`; }).join('')}</div>` : '—'}</td><td>${result ? teacherResultMarkup(result) : '—'}</td></tr>`;
  }).join('') || '<tr><td colspan="6" class="empty-state">Ученики пока не добавлены.</td></tr>'}</tbody></table></section><dialog id="student-dialog"><form id="student-form" class="dialog-form"><div><p class="eyebrow">Новый ученик</p><h2>Добавить ученика</h2></div><label>Имя<input name="name" maxlength="120" autocomplete="off" required></label><p id="student-form-error" class="form-error" role="alert"></p><div class="homework-actions"><button class="primary-button" type="submit">Сохранить</button><button id="cancel-student" class="secondary-button" type="button">Отмена</button></div></form></dialog><dialog id="teacher-worksheet-preview" class="preview-dialog"><div id="teacher-worksheet-preview-content"></div><button id="close-teacher-worksheet-preview" class="secondary-button" type="button">Закрыть предпросмотр</button></dialog><dialog id="teacher-result-review" class="preview-dialog"><div id="teacher-result-review-content"></div><label class="teacher-comment-field">Комментарий преподавателя<textarea id="teacher-review-comment" rows="4" placeholder="Что нужно исправить или на что обратить внимание"></textarea></label><div class="review-dialog-actions"><button id="complete-result-review" class="primary-button" type="button">Принять</button><button id="return-result-review" class="secondary-button" type="button">Вернуть на доработку</button><button id="close-result-review" class="secondary-button" type="button">Закрыть</button></div></dialog>`;
  bindTeacherProgressControls(); document.querySelectorAll('.teacher-homework-link, .open-teacher-homework').forEach((button) => button.addEventListener('click', () => openTeacherHomeworkPreview(button.dataset.homeworkId))); document.querySelectorAll('.review-homework-result').forEach((button) => button.addEventListener('click', () => openTeacherResultReview(button.dataset.resultId))); document.querySelectorAll('.delete-homework').forEach((button) => button.addEventListener('click', () => deleteStudentHomework(button.dataset.homeworkId))); el('add-student').addEventListener('click', () => el('student-dialog').showModal()); el('cancel-student').addEventListener('click', () => el('student-dialog').close()); el('student-form').addEventListener('submit', saveStudent); el('close-teacher-worksheet-preview').addEventListener('click', () => el('teacher-worksheet-preview').close()); el('close-result-review').addEventListener('click', () => el('teacher-result-review').close()); animateBars();
}
...
async function deleteStudentHomework(homeworkId) {
  ...
  try {
    ...
    if (results.items.some((item) => item.status === 'needs_review')) { toast('Сначала завершите проверку результата. Нажмите «Проверить».'); return; }
...
async function openTeacherResultReview(resultId) {
  const result = state.teacherData.results.find((item) => item.id === resultId);
  const homework = result && state.teacherData.homework.find((item) => item.id === result.homework);
  const student = result && state.students.find((item) => item.id === result.student);
  if (!result || !homework || !student) { toast('Результат проверки не найден.'); return; }
  setLoading(true);
  try {
    const [worksheet, exercises] = homework.worksheet ? await Promise.all([
      pb.collection('worksheets').getOne(homework.worksheet, { requestKey: null }),
      pb.collection('worksheet_exercises').getFullList({ filter: `worksheet="${homework.worksheet}"`, sort: 'order', requestKey: null }),
    ]) : [null, []];
    const exerciseById = new Map(exercises.map((exercise) => [exercise.id, exercise]));
    const answers = Array.isArray(result.open_answers) ? result.open_answers : [];
    el('teacher-result-review-content').innerHTML = `<section class="result-review-view"><p class="eyebrow">Проверка открытых ответов</p><h2>${escapeHtml(worksheet?.title || homework.title || 'Worksheet')}</h2><div class="result-review-meta"><span><b>Ученик:</b> ${escapeHtml(student.name)}</span><span><b>Автопроверка:</b> ${Number(result.score || 0)}/${Number(result.max_score || 0)} · ${Number(result.percentage || 0)}%</span></div>${answers.length ? answers.map((answer, index) => { const exercise = exerciseById.get(answer.exercise_id); const content = exerciseContent(exercise); const instruction = exercise?.instruction || answer.prompt || ''; const prompt = content.prompt && content.prompt !== instruction ? content.prompt : ''; const support = typeof content.support === 'string' ? content.support : ''; const successCriterion = content.success_criterion || (Array.isArray(content.success_criteria) ? content.success_criteria.join('\n') : ''); return `<article class="result-review-answer"><h3>Открытый ответ ${index + 1}</h3>${instruction ? `<p><b>Инструкция:</b> ${escapeHtml(instruction)}</p>` : ''}${prompt ? `<p><b>Задание:</b> ${escapeHtml(prompt)}</p>` : ''}${support ? `<p><b>Опора:</b> ${escapeHtml(support)}</p>` : ''}${successCriterion ? `<p><b>Критерий успеха:</b> ${escapeHtml(successCriterion)}</p>` : ''}<div class="student-open-answer"><b>Ответ ученика</b><p>${escapeHtml(answer.answer || 'Ответ не указан')}</p></div></article>`; }).join('') : '<p class="empty-state">Открытые ответы не найдены.</p>'}</section>`;
    el('teacher-review-comment').value = result.teacher_comment || '';
    const completeButton = el('complete-result-review'), returnButton = el('return-result-review');
    completeButton.disabled = result.status !== 'needs_review'; returnButton.disabled = result.status !== 'needs_review';
    completeButton.onclick = () => completeTeacherResultReview(result.id, 'reviewed'); returnButton.onclick = () => completeTeacherResultReview(result.id, 'returned');
    el('teacher-result-review').showModal();
  } catch (error) { handleFatal(error); } finally { setLoading(false); }
}

async function completeTeacherResultReview(resultId, status) {
  setLoading(true);
  try {
    const teacherComment = el('teacher-review-comment').value.trim();
    const updated = await pb.collection('homework_results').update(resultId, { status, teacher_comment: teacherComment });
    state.teacherData.results = state.teacherData.results.map((item) => item.id === resultId ? updated : item);
    el('teacher-result-review').close(); renderTeacher(); toast(status === 'returned' ? 'Ответ возвращён на доработку' : 'Проверка завершена');
  } catch (error) { handleFatal(error); } finally { setLoading(false); }
}
...
function teacherResultMarkup(result) { if (result.status !== 'needs_review') return `<strong>${result.status === 'reviewed' ? 'Проверено · ' : ''}${result.percentage}%</strong><div class="muted">${formatDate(result.completed_at)}</div>`; return `<strong class="review-status">Ожидает проверки</strong>${Number(result.max_score) ? `<div class="muted">Автопроверка: ${result.percentage}%</div>` : ''}<div class="muted">${formatDate(result.completed_at)}</div><button class="primary-button review-homework-result review-result-button" data-result-id="${result.id}" type="button">Проверить</button>`; }
...
ВАЖНО: worksheet должен быть полностью self-contained; ни один правильный ответ не должен зависеть от невидимой картинки или скрытой информации source.
...
async function renderWorksheetPlayer() {
  ...
  const studentTasks = state.exercises.filter(isStudentFacingExercise); setHeader(worksheet.title, worksheet.instructions || 'Выполните все упражнения.', `0 из ${studentTasks.length}`);
  const existingResult = state.result?.homework === state.homework.id ? state.result : null;
  const savedStatus = existingResult?.status === 'needs_review' ? 'Ответ отправлен преподавателю' : existingResult?.status === 'returned' ? `Нужно исправить${existingResult.teacher_comment ? `: ${existingResult.teacher_comment}` : ''}` : existingResult ? `Сохранённый результат: ${existingResult.score}/${existingResult.max_score} · ${existingResult.percentage}%` : '';
  el('content').innerHTML = worksheetPageMarkup(worksheet, state.exercises, false, savedStatus);
  bindWorksheetInteractions(el('worksheet-player')); el('worksheet-player').addEventListener('submit', checkWorksheet); updateWorksheetProgress();
}

function worksheetPageMarkup(worksheet, exercises, preview = false, savedStatus = '') { const studentTasks = exercises.filter(isStudentFacingExercise), characterCards = characterCardsFrom(worksheet, exercises), introText = String(worksheet.intro_text || '').trim(); return `<form ${preview ? '' : 'id="worksheet-player"'} class="worksheet-page student-worksheet"><header class="worksheet-intro">${preview && worksheet.id ? `<p class="worksheet-label">Worksheet ID: ${escapeHtml(worksheet.id)}</p>` : ''}<p class="worksheet-label">Interactive worksheet</p><h2>${escapeHtml(worksheet.title || 'Worksheet')}</h2>${worksheet.instructions ? `<p class="worksheet-goal">${escapeHtml(worksheet.instructions)}</p>` : ''}<div class="worksheet-meta">${worksheet.estimated_time ? `<span>Время: ${escapeHtml(worksheet.estimated_time)}</span>` : ''}<span id="worksheet-progress">Выполнено 0 из ${studentTasks.length}</span></div></header>${introText ? `<section class="worksheet-intro-text">${escapeHtml(introText).replace(/\n/g, '<br>')}</section>` : ''}${characterCardsMarkup(characterCards)}<main class="worksheet-tasks">${exercises.map((exercise, index) => worksheetExerciseMarkup(exercise, index, preview)).join('')}</main><footer class="worksheet-submit"><button class="primary-button" type="${preview ? 'button' : 'submit'}" ${!preview && studentTasks.length ? 'disabled' : ''}>Finish worksheet</button><strong id="worksheet-score">${escapeHtml(savedStatus)}</strong></footer></form>`; }
...
function exerciseDataError(exercise) {
  ...
  if (exercise.type === 'open_text_teacher_review' && (typeof content.support !== 'string' || !content.support.trim() || content.teacher_reviewed !== true)) return `Exercise data is invalid: ${exercise.type}`;
  return '';
}
function worksheetExerciseMarkup(exercise, index, teacherPreview = false) {
  ...
  const dataError = exerciseDataError(exercise); if (dataError) { console.error(dataError, exercise); return `<section class="worksheet-exercise task-section exercise-data-error" data-exercise="${escapeAttr(exercise.id || '')}"><div class="task-heading"><div><span class="task-label">Task ${index + 1}</span><h3>${escapeHtml(exercise.instruction || exercise.type || 'Exercise')}</h3></div></div><p class="feedback">${escapeHtml(teacherPreview ? dataError : 'This task is unavailable.')}</p></section>`; }
...
function bindWorksheetInteractions(root) { root.querySelectorAll('.word-token').forEach((button) => button.addEventListener('click', toggleWordToken)); root.querySelectorAll('.match-item').forEach((button) => button.addEventListener('click', selectMatchItem)); root.querySelectorAll('.worksheet-answer input, .worksheet-answer select, .worksheet-answer textarea').forEach((input) => { input.addEventListener('change', updateWorksheetProgress); input.addEventListener('input', updateWorksheetProgress); }); bindDragDrop(root); }
...
function isAssessableExercise(exercise) { return !exerciseDataError(exercise) && ['multiple_choice', 'text_input', 'reorder_words', 'matching', 'dropdown', 'drag_drop'].includes(exercise.type) && Number(exercise.points || 0) > 0; }
function isStudentFacingExercise(exercise) { return !exerciseDataError(exercise) && (isAssessableExercise(exercise) || exercise.type === 'open_text_teacher_review'); }
...
function isExerciseComplete(exercise) { const section = document.querySelector(`[data-exercise="${exercise.id}"]`); if (!section) return false; if (exercise.type === 'open_text_teacher_review') return !!section.querySelector('textarea')?.value.trim(); if (exercise.type === 'multiple_choice') return exerciseItems(exercise).every((_, index) => section.querySelector(`input[name="ws-${exercise.id}-${index}"]:checked`)); if (exercise.type === 'text_input') return !!section.querySelector('input')?.value.trim(); if (exercise.type === 'reorder_words') { const items = section.querySelectorAll('.reorder-item'); return items.length > 0 && [...items].every((item) => item.querySelectorAll('.word-answer .word-token').length > 0); } if (exercise.type === 'dropdown') { const selects = [...section.querySelectorAll('.dropdown-sentence select')]; return selects.length > 0 && selects.every((select) => select.value !== ''); } if (exercise.type === 'drag_drop') { const items = section.querySelectorAll('.drag-item').length; return items > 0 && section.querySelectorAll('.drop-zone .drag-item').length === items; } return section.querySelectorAll('.match-left.paired').length === section.querySelectorAll('.match-left').length; }
function updateWorksheetProgress() { const player = el('worksheet-player'); if (!player) return; const studentTasks = state.exercises.filter(isStudentFacingExercise), completed = studentTasks.filter(isExerciseComplete).length; el('page-kicker').textContent = `${completed} из ${studentTasks.length}`; const indicator = el('worksheet-progress'); if (indicator) indicator.textContent = `Выполнено ${completed} из ${studentTasks.length}`; const finish = player.querySelector('[type="submit"]'); if (finish) finish.disabled = studentTasks.length > 0 && completed !== studentTasks.length; }

async function checkWorksheet(event) {
  event.preventDefault(); let score = 0, maxScore = 0; const reviewExercises = state.exercises.filter((exercise) => exercise.type === 'open_text_teacher_review' && !exerciseDataError(exercise)), openAnswers = reviewExercises.map((exercise) => ({ exercise_id: exercise.id, prompt: exercise.content?.prompt || exercise.instruction || '', answer: document.querySelector(`[data-exercise="${exercise.id}"] textarea`).value.trim() }));
