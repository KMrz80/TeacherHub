...
function buildAgentPrompt() {
  ...
  const values = [
    ...
    ['Название', form.elements.worksheet_title.value.trim()],
...
function showBuilderPreview() { const nodes = [...document.querySelectorAll('.exercise-editor')], editorExercises = nodes.map((node, order) => exerciseDataForSave(node, order)).filter(Boolean), exercises = editorExercises.length ? editorExercises : (state.previewExercises || []); const form = el('worksheet-form'); el('worksheet-preview-content').innerHTML = worksheetPageMarkup({ id: state.editingWorksheet?.id || '', title: form.elements.worksheet_title.value || 'Worksheet', instructions: form.elements.work_goal.value, intro_text: form.elements.intro_text.value, estimated_time: state.editingWorksheet?.estimated_time || form.elements.estimated_time.value }, exercises, true); bindWorksheetInteractions(el('worksheet-preview-content')); el('worksheet-preview-dialog').showModal(); }

function worksheetDataFromForm(status = 'draft') { const form = el('worksheet-form'), due = form.elements.due_date.value, workGoal = form.elements.work_goal.value.trim(); return { student: form.elements.student.value, title: form.elements.worksheet_title.value.trim(), instructions: workGoal, focus: workGoal, intro_text: form.elements.intro_text.value.trim(), estimated_time: form.elements.estimated_time.value.trim(), status, due_date: due ? new Date(due).toISOString() : '', created_by: pb.authStore.record.id }; }
...
async function saveWorksheet(event) {
  ...
  if (!form.elements.worksheet_title.value.trim()) { showError('Заполните название worksheet.'); form.elements.worksheet_title.focus(); return; }
